...
CREATE TABLE IF NOT EXISTS imoveis (
    ...
    tipo_imovel TEXT NOT NULL DEFAULT 'Casa',  -- TipoImovel
...
CREATE TABLE IF NOT EXISTS pessoas (
    ...
    -- Validações
    CHECK(situacao IN ('Inquilino', 'Fiador', 'Ambos')),
...
BEGIN
    ...
    AND NEW.status_contrato = 'Ativo';
END;

CREATE TRIGGER IF NOT EXISTS atualizar_imovel_ocupado_update
AFTER UPDATE ON contratos
BEGIN
    -- Marcar como ocupado se status for Ativo
    UPDATE imoveis 
    SET ocupado = 'Sim'
    WHERE id = NEW.id_imovel 
    AND NEW.status_contrato = 'Ativo';
    
    -- Marcar como disponível se não houver contratos ativos
    UPDATE imoveis
    SET ocupado = 'Não'
    WHERE id = NEW.id_imovel
